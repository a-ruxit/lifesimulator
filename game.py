    ...
    for i in range(grid_size):
        for j in range(grid_size):
            if board[i][j] == DEAD:
                ...
                eye_color = (0, 0, 0)
                if board[i][j].vision == 3:
                    eye_color = (255, 255, 255)
                if board[i][j].vision == 5:
                    eye_color = (255, 223, 0)
                if board[i][j].face == 1:
                    pygame.draw.circle(screen, eye_color, (i * cell_size + 2, j * cell_size + 2), 1)
                    pygame.draw.circle(screen, eye_color, (i * cell_size + 6, j * cell_size + 2), 1)
                elif board[i][j].face == 2:
                    pygame.draw.circle(screen, eye_color, (i * cell_size + 6, j * cell_size + 2), 1)
                    pygame.draw.circle(screen, eye_color, (i * cell_size + 6, j * cell_size + 6), 1)
                elif board[i][j].face == 3:
                    pygame.draw.circle(screen, eye_color, (i * cell_size + 2, j * cell_size + 6), 1)
                    pygame.draw.circle(screen, eye_color, (i * cell_size + 6, j * cell_size + 6), 1)
...
def edit_json(agent_list):
    agent_dict = {}
    for agent in agent_list:
        agent_dict[f'{agent}'] = {'Live':agent.live, 'Energy':agent.energy, 'Vision':agent.vision}
